...
st.markdown("This planner assigns optimal land across crops in each field to maximize profit while considering water and fertilizer constraints.")
...
alpha = 0.2  # water scarcity weight
...
if st.button("🚀 Optimize Now"):
    model = LpProblem("Crop_Planning_Final", LpMaximize)
    ...
    for f in fields:
        for c in crops:
            ...
            # ✅ FIXED LOGIC: real profit - fertilizer cost
            profit = (Y * P) - (F * fertilizer_cost)
            penalty = alpha * (W * (1 - RI))  # only water penalty
    ...
    if model.status == 1:
        ...
        st.error("❌ Optimization failed. Try increasing water/rainfall or reducing area.")
